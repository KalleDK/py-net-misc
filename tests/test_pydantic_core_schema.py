...
try:
    ...
    from net_misc.macaddress import MacAddressBase, MacFormat
except ImportError:
...
def test_get_pydantic_core_schema_validates_mac_address_with_format(format_type: MacFormat) -> None:
    adapter: TypeAdapter[MacAddressBase] = TypeAdapter(Annotated[MacAddressBase, format_type])

    result = adapter.validate_python("00:1a:2b:3c:4d:5e")

    assert result == MacAddressBase("00:1a:2b:3c:4d:5e")
    assert isinstance(result, MacAddressBase)
